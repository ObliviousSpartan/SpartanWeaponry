...
public class MobOilEffect extends OilEffect 
{
	...
	public MobOilEffect(String nameIn, int colorIn, int maxUsesIn, float damageModifierIn, IUsePredicate usePredicateIn, MobEffect mobEffectIn, int effectDurationIn, int effectLevelIn) 
	{
	...
	public void getTooltip(ItemStack stackIn, List<Component> tooltipListIn)
	{
		...
		if(effectLevel > 0)
			mobEffectComponent.append(" ").append(new TranslatableComponent("enchantment.level." + Integer.toString(effectLevel + 1)));
		if(damageModifier == 0.0f)
			tooltipListIn.add(new TranslatableComponent("tooltip." + ModSpartanWeaponry.ID + ".weapon_oil.applied." + name, mobEffectComponent, (float)effectDuration / 20.0f).withStyle(ChatFormatting.BLUE));			
		else
			tooltipListIn.add(new TranslatableComponent("tooltip." + ModSpartanWeaponry.ID + ".weapon_oil.applied." + name, (getDamageModifier() * 100.0f), mobEffectComponent, (float)effectDuration / 20.0f).withStyle(ChatFormatting.BLUE));
	}
}
